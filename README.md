...
| 項目 | 用途 |
|------|------|
| [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) | AWS リソースの操作 |
| [AWS SAM CLI](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/install-sam-cli.html) | サーバーレスアプリのビルド・デプロイ |
...
2. Bot の **Application ID** と **Public Key** を控える
3. OAuth2 > URL Generator で `applications.commands` スコープを選択し、生成された URL でサーバーに Bot を招待
4. Slash Commands を登録する（`/add`, `/list`, `/delete`, `/feeds`）
...
`sam deploy --guided` で以下を設定します。

| 設定項目 | 説明 |
|----------|------|
| Stack Name | CloudFormation スタック名（例: `rss-generator`） |
| AWS Region | デプロイ先リージョン（Bedrock が利用可能なリージョンを選択） |
| Confirm changes before deploy | `Yes` 推奨 |
| Allow SAM CLI IAM role creation | `Yes`（Lambda 実行ロール作成に必要） |

設定は `samconfig.toml` に保存され、次回以降は `sam build && sam deploy` のみで実行できます。

### 5. Discord Interactions Endpoint の設定

1. デプロイ後の出力（Outputs）から **API Gateway のエンドポイント URL** を取得
2. Discord Developer Portal > アプリケーション > General Information > **Interactions Endpoint URL** に設定
3. Discord が検証リクエストを送信し、成功すれば設定完了

### 6. GitHub Actions の設定（CI/CD）

OIDC 連携により長期 IAM アクセスキーは不要です。

#### 6a. IAM OIDC プロバイダーの作成
...
#### 6b. GitHub Actions 用 IAM ロールの作成
