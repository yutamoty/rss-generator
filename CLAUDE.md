    ...
    → Step Functions (Standard)
      ...
      → Map (並列): サイトごとに
        → Lambda (generate-feed):
          ...
          → Bedrock (Gemma 4 31B) で記事一覧を構造化抽出
...
| Bedrock (Gemma 4 31B) | Markdown→記事一覧の構造化抽出 |
| S3 | Atom XML の配置 |
...
2. **Map (並列)**: サイトごとに Lambda (generate-feed) を実行
   ...
   - Bedrock (Gemma 4 31B) で記事一覧を構造化抽出 (title, link, date, summary)
   - Atom XML 生成 → S3 PUT
...
```bash
# GitHub Actions による自動デプロイ（初回含む）
# main ブランチへの push で sam build && sam deploy が実行される
# samconfig.toml をリポジトリに含めておくこと
